...
export default function Game() {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      ...
      if (savedLevel && savedSubjectId && savedDifficulty) {
        ...
        // Auto-resume if they had progress
  ...
  const confirmReset = () => {
    localStorage.clear(); // Clear persistence on manual reset
    setScore(0);
    setHistory([]);
    setGameState('grade-selection');
  ...
  const handleSignOut = () => {
    localStorage.clear(); // Clear persistence on logout
    signOut();
  };
  ...
  const renderAuthGate = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-10 text-center py-10">
      <div className="space-y-4">
        ...
        <h1 className="text-4xl font-black text-zinc-900 tracking-tight">STEM BLAST!</h1>
        <p className="text-zinc-500 font-medium px-10">Sign in to track your score, unlock levels, and challenge the AI.</p>
      </div>
      <div className="flex flex-col gap-4 px-6">
        <button onClick={() => signIn('google')} className="w-full py-5 bg-white border-2 border-zinc-100 rounded-3xl flex items-center justify-center gap-4 font-bold text-zinc-700 shadow-sm hover:bg-zinc-50 transition-all">
    ...
    <div className="w-full max-w-md mx-auto h-full flex flex-col relative overflow-hidden bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-white/20">
      ...
      <AnimatePresence>
        {showAdFullscreen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[200] bg-black flex flex-col items-center justify-center p-8 text-center text-white">
            <button onClick={() => setShowAdExitConfirm(true)} className="absolute top-8 right-8 p-3 bg-white/10 rounded-full"><X className="w-6 h-6" /></button>
            <PlayCircle className="w-20 h-20 text-purple-500 animate-pulse mb-6" />
            <h2 className="text-2xl font-black uppercase mb-4">Sponsor Message</h2>
            <div className="text-4xl font-black">{adTimer}s</div>
            {showAdExitConfirm && (
              <div className="absolute inset-0 z-[210] bg-black/95 flex items-center justify-center p-6">
                <div className="bg-zinc-900 p-8 rounded-3xl border border-white/10">
                  <p className="mb-6">Exit early and lose your hint?</p>
                  <button onClick={() => setShowAdExitConfirm(false)} className="w-full py-4 bg-purple-600 rounded-xl font-bold mb-3">Keep Watching</button>
                  <button onClick={confirmExitAd} className="w-full py-4 bg-zinc-800 rounded-xl">Exit</button>
                </div>
              </div>
            )}
          </motion.div>
      ...
      {gameState !== 'auth-gate' && (
        <div className="flex justify-between items-center p-6 pb-2">
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleSignOut} className="p-2 bg-zinc-100 rounded-full text-zinc-600 hover:bg-red-50 hover:text-red-600 transition-all"><LogOut className="w-4 h-4" /></button>
          </div>
          {gameState === 'playing' && (
        ...
        <AnimatePresence mode="wait">
          ...
          {gameState === 'grade-selection' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-8 text-center">
              <h1 className="text-3xl font-black">Choose Grade</h1>
              <div className="grid gap-3 max-h-[400px] overflow-y-auto pr-2">
                {LEVELS.map(lvl => <button key={lvl} onClick={() => { setLevel(lvl); setGameState('subject-selection'); }} className="p-5 rounded-3xl border-2 font-bold hover:border-blue-400 bg-white">{lvl}</button>)}
              </div>
            </motion.div>
          )}
          {gameState === 'subject-selection' && (
            <motion.div initial={{ x: 20 }} animate={{ x: 0 }} className="flex flex-col gap-8 text-center">
              <h1 className="text-3xl font-black">Pick Subject</h1>
              <div className="grid grid-cols-2 gap-4">
                {SUBJECTS.map(sub => <button key={sub.id} onClick={() => { setSubject(sub); setGameState('difficulty-selection'); }} className="p-6 rounded-3xl border-2 flex flex-col items-center gap-3 bg-white"><sub.icon className={sub.color} /> <span className="font-bold">{sub.name}</span></button>)}
              </div>
            </motion.div>
          )}
          {gameState === 'difficulty-selection' && (
            <motion.div initial={{ x: 20 }} animate={{ x: 0 }} className="flex flex-col gap-8 text-center">
              <h1 className="text-3xl font-black">Select Level</h1>
              <div className="grid gap-4">
                {DIFFICULTIES.map(diff => <button key={diff} onClick={() => { setDifficulty(diff); setGameState('playing'); fetchQuestion(level, subject.name, score, diff); }} className="p-6 rounded-3xl border-2 font-black text-xl bg-white hover:border-blue-400">{diff}</button>)}
              </div>
            </motion.div>
          )}
          {gameState === 'playing' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
              {loading || !question ? <div className="flex-1 flex flex-col items-center justify-center gap-4 py-20 animate-pulse"><RefreshCw className="animate-spin text-blue-500" /> <p className="text-[10px] font-bold text-zinc-400">Consulting AI...</p></div> : (
                <>
                  <div className="bg-white rounded-3xl p-8 shadow-sm mb-6 text-center border-2 border-zinc-50 min-h-[220px] flex flex-col justify-center items-center">
                  ...
                  <div className="grid gap-3">
                    {question.options.map((opt: string, i: number) => {
                      ...
                      let variant = "bg-white border-zinc-100";
                      if (isFound) variant = "bg-green-500 border-green-600 text-white shadow-lg";
                      else if (isWrong) variant = "bg-zinc-50 text-zinc-300 opacity-50";
                      return <button key={i} onClick={() => handleAnswer(opt)} className={`p-5 rounded-2xl font-bold text-lg border-2 text-left ${variant}`}>{opt}</button>;
                    })}
                  </div>
                  <div className="mt-8 flex gap-3 pb-4">
                    <button onClick={() => handleGetHint()} disabled={!!hint} className="flex-1 p-4 bg-yellow-400 rounded-2xl font-black text-xs">50 PTS HINT</button>
                    {!isPremium && <button onClick={startAdReward} className="flex-1 p-4 bg-purple-600 text-white rounded-2xl font-black text-xs">AD HINT</button>}
                  </div>
                </>
              )}
            </motion.div>
      ...
      {!isPremium && gameState !== 'auth-gate' && (
        <div className="p-4 bg-zinc-50 border-t border-zinc-100">
          <button onClick={() => setShowPaymentModal(true)} className="w-full py-4 rounded-2xl bg-zinc-900 text-white font-bold text-xs flex items-center justify-center gap-2 hover:bg-black transition-all">
