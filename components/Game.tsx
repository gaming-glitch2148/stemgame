...
export default function Game() {
  ...
  const fetchQuestion = async (selectedLevel = level, currentSubject = subject.name, currentScore = score, currentDiff = difficulty) => {
    ...
    try {
      ...
      if (data && data.question) {
        // CLEANUP: Remove any "Q[number]:" prefixes from CSV questions
  ...
  const handleAnswer = (answer: string) => {
    // SECURITY: Ensure click logic is robust
  ...
  const renderAuthGate = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-10 text-center py-10">
      ...
      <div className="flex flex-col gap-4 px-6">
        <button onClick={() => signIn('google')} className="w-full py-5 bg-white dark:bg-zinc-800 border-2 border-zinc-100 dark:border-zinc-700 rounded-3xl flex items-center justify-center gap-4 font-bold text-zinc-700 dark:text-zinc-200 transition-all hover:bg-zinc-50"><img src="https://www.google.com/favicon.ico" className="w-5 h-5" alt="Google" /> Continue with Google</button>
        <button onClick={() => signIn('facebook')} className="w-full py-5 bg-[#1877F2] text-white rounded-3xl flex items-center justify-center gap-4 font-bold transition-all hover:bg-[#166fe5]"><img src="https://www.facebook.com/favicon.ico" className="w-5 h-5 brightness-0 invert" alt="Facebook" /> Continue with Facebook</button>
      </div>
    </motion.div>
  );
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[200] bg-black flex flex-col items-center justify-center p-8 text-center text-white">
            ...
            {showAdExitConfirm && (
              <div className="absolute inset-0 z-[210] bg-black/95 flex items-center justify-center p-6">
                <div className="bg-zinc-900 p-8 rounded-3xl border border-white/10 shadow-2xl max-w-xs"><p className="mb-6 text-zinc-300 text-sm">Exit early and lose your hint?</p><button onClick={() => setShowAdExitConfirm(false)} className="w-full py-4 bg-purple-600 text-white rounded-xl font-bold mb-3">Keep Watching</button><button onClick={confirmExitAd} className="w-full py-4 bg-zinc-800 text-zinc-400 rounded-xl">Exit</button></div>
              </div>
            )}
          </motion.div>
        )}
      ...
      {gameState !== 'auth-gate' && (
        <div className="flex justify-between items-center p-6 pb-2">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-yellow-100 dark:bg-yellow-900/30 px-4 py-2 rounded-full border border-yellow-200 dark:border-yellow-700/50"><Trophy className="w-5 h-5 text-yellow-600 dark:text-yellow-400" /><span className="font-bold text-yellow-700 dark:text-yellow-300">{score}</span></div>
        ...
        <AnimatePresence mode="wait">
          ...
          {gameState === 'playing' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
              {loading || !question ? <div className="flex-1 flex flex-col items-center justify-center gap-4 py-20 animate-pulse"><RefreshCw className="animate-spin text-blue-500" size={48} /> <p className="text-sm font-bold text-zinc-400">Consulting AI...</p></div> : (
                <>
                  ...
                  <div className="mt-8 flex gap-3 pb-4">
                    <button onClick={() => handleGetHint()} disabled={!!hint || isCorrect} className="flex-1 p-4 bg-yellow-400 text-yellow-900 rounded-2xl font-black text-xs hover:bg-yellow-500 active:scale-95 transition-all disabled:opacity-50 shadow-md">50 PTS HINT</button>
