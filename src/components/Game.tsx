...
export default function Game() {
  ...
  // Sound effect simulation (visual only for now, but placeholders for audio)
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (correct) {
      ...
      setScore(s => s + 10);
      setTimeout(fetchQuestion, 2000); // Auto next
    } else {
      playSound('wrong');
      setTimeout(() => {
    ...
    <div className="w-full max-w-md mx-auto h-full flex flex-col relative overflow-hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20">
      ...
      {!isPremium && (
        <div className="bg-zinc-100 dark:bg-zinc-950 p-4 border-t border-zinc-200 dark:border-zinc-800">
          <div className="bg-zinc-200 dark:bg-zinc-800 rounded-lg p-3 text-center mb-3">
            <p className="text-[10px] uppercase text-zinc-400 font-bold tracking-widest mb-1">Advertisement</p>
            <div className="h-12 bg-zinc-300 dark:bg-zinc-700 rounded flex items-center justify-center text-zinc-500 text-sm">
              Example Ad Banner
            </div>
          </div>
